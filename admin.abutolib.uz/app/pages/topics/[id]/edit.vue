  ...
  <NuxtLayout name="admin">
    ...
    <div v-if="pending" class="flex items-center justify-center h-64">
      <div class="flex flex-col items-center gap-3">
        <svg class="w-7 h-7 text-primary-500 animate-spin" fill="none" viewBox="0 0 24 24">
          <circle class="opacity-25" cx="12" cy="12" r="10" stroke="currentColor" stroke-width="4" />
          <path class="opacity-75" fill="currentColor" d="M4 12a8 8 0 018-8V0C5.373 0 0 5.373 0 12h4z" />
        </svg>
        <p class="text-sm text-gray-400 dark:text-gray-500">Loading topic…</p>
      </div>
    </div>

    <div v-else-if="topic" class="max-w-5xl mx-auto px-4 sm:px-6 py-6 space-y-6">
      ...
      <div class="flex items-center gap-3">
        ...
        <div>
          <h2 class="text-base font-semibold text-gray-900 dark:text-white">Edit Topic</h2>
          <p class="text-xs text-gray-400 dark:text-gray-500 truncate max-w-xs sm:max-w-sm">{{ form.name }}</p>
        </div>
      </div>
...
const { data: response, pending } = await useAsyncData(
  ...
  () => api.get<any>(`/admin/topics/${route.params.id}`)
)
...
watch(topic, (t) => {
  if (!t) return
  form.subject_id = t.subject_id
  form.name = t.name
  form.description = t.description ?? ''
  form.order = t.order ?? 0
  form.time_limit = t.time_limit ?? null
  form.is_active = !!t.is_active
  questions.value = (t.questions ?? []).map((q: any) => ({
    id: q.id,
    question: q.question,
    ...
    match_pairs: (q.match_pairs ?? []).map((p: any) => ({
      id: p.id,
      left: p.left,
      right: p.right,
    })),
...
async function save() {
  ...
  saving.value = true
  try {
    await api.put(`/admin/topics/${route.params.id}`, buildPayload())
    toast.add({ title: 'Topic updated', color: 'success', icon: 'i-heroicons-check-circle' })
    router.push('/topics')
  } catch (e: any) {
    toast.add({ title: e?.data?.message ?? 'Failed to update topic', color: 'error', icon: 'i-heroicons-x-circle' })
  } finally {
    saving.value = false
  }
}
