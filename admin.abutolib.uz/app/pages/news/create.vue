  ...
  <NuxtLayout name="admin">
    <div class="max-w-4xl space-y-6">
      <div class="flex items-center gap-3">
        ...
        <div>
          <h2 class="text-base font-semibold text-gray-900 dark:text-white">New Article</h2>
          <p class="text-xs text-gray-400">Create a new news article</p>
        </div>
      </div>
...
async function save() {
  ...
  if (!form.body.trim()) {
    toast.add({ title: 'Body is required', color: 'error', icon: 'i-heroicons-x-circle' })
    return
  }
  ...
  try {
    ...
    toast.add({ title: 'Article created', color: 'success', icon: 'i-heroicons-check-circle' })
    router.push('/news')
  } catch (e: any) {
    toast.add({ title: e?.data?.message ?? 'Failed to create article', color: 'error', icon: 'i-heroicons-x-circle' })
  } finally {
    saving.value = false
  }
}
